...
public class EventItemInfo
{
    ...
    public void getTooltip(ItemTooltipEvent event)
    {
        ...
        if (damageIndex != -1)
        {
            ...
            {
                int damage = (int) ((AttributeModifier) stack.getAttributeModifiers().get(SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName()).toArray()[0])
                        .getAmount();

                toolTip.set(
                        damageIndex,
                        EnumChatFormatting.BLUE + "+" + damage + "-" + (damage + TWSettings.DAMAGE_MODIFIER_MACE) + " "
                                + StatCollector.translateToLocal("attribute.name.generic.attackDamage") + EnumChatFormatting.RESET);
            }
        }
    }
}
