...
public class ItemHammer extends ItemWeapon
{
    ...
    {
        int charge = (getMaxItemUseDuration(stack) - i) / 2;
        int explosionSize = 6;
        ...
        if (charge > 1)
        {
            ...
            int i1 = MathHelper.floor_double(x - explosionSize - 1.0D);
            int i2 = MathHelper.floor_double(y - explosionSize - 1.0D);
            int i3 = MathHelper.floor_double(z - explosionSize - 1.0D);
            int i4 = MathHelper.floor_double(x + explosionSize + 1.0D);
            int i5 = MathHelper.floor_double(y + explosionSize + 1.0D);
            int i6 = MathHelper.floor_double(z + explosionSize + 1.0D);

            int knockBackLvl = EnchantmentHelper.getEnchantmentLevel(Enchantment.knockback.effectId, stack) + 1;

            List<Entity> targets = player.worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getAABBPool().getAABB(i1, i2, i3, i4, i5, i6));
            ...
            {
                Entity entity = targets.get(k2);
                double d7 = entity.getDistance(x, y, z) / explosionSize;
